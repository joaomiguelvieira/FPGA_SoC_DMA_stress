...
int main(int argc, char **argv) {
	...
	for (int i = 0; i < N_RUNS; i++) {
#ifndef FAST_MODE
		xil_printf("Performing 128-bit PS to PL transfer...\n\r");
#endif
		XTime_GetTime(&start);
		ps2pl2();
		XTime_GetTime(&end);
	...
	for (int i = 0; i < N_RUNS; i++) {
#ifndef FAST_MODE
		xil_printf("Performing 128-bit PS to PL optimized transfer...\n\r");
#endif
		XTime_GetTime(&start);
		ps2pl2_opt();
		XTime_GetTime(&end);
